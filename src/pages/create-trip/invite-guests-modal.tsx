...
interface InviteGuestsModalProps {
  closeGuesteModal: () => void,
  emailsToInvite: string[],
  addNewEmailToInvite: (event: FormEvent<HTMLFormElement>) => void,
  removeEmailFromInvites: (email: string) => void
}

export function InviteGuestsModal({ closeGuesteModal, emailsToInvite, removeEmailFromInvites, addNewEmailToInvite }: InviteGuestsModalProps) {
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center">
      <div className="w-[640px] roundex-xl py-5 px-6 shadow-shape bg-zinc-900 space-y-5">
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold">Selecionar convidados</h2>
            <button onClick={closeGuesteModal}>
        ...
        <div className="flex flex-wrap gap-2">
          {emailsToInvite.map(email => {
            return (
              <div key={email} className="py-1.5 px-2.5 roundec-md bg-zinc-800 flex items-center gap-2">
